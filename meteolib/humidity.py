...
from .constants import pzero, Ttriple, Lv, cp
from .temperature import _to_K, _to_C, Tpot
...
def magnus_w(t, Kelvin=None, p=None, hPa=False):
  ...
  :return: saturation pressure in hPa. If `t` is not a valid number, ``np.nan``
    is returned.
  :rtype: float
  :raises ValueError: if `Kelvin` is not ``True``, ``False``, ``None``, or missing
  :raises ValueError: if `hPa` is not ``True``, ``False``, ``None``, or missing
...
def hyland_wexler_w(t, Kelvin=None, p=None, hPa=False):
  '''
  Formula by Hyland and Wexler, identical to :py:meth:`meltib.humidity.magnus_w`,
...
def hyland_wexler_i(t, Kelvin=None, p=None, hPa=False):
  '''
  Formula by Hyland and Wexler, identical to :py:meth:`meltib.humidity.magnus_i`,
  except formula provided by [HyW1983]_:
...
def sonntag_w(t, Kelvin=None, p=None, hPa=False):
  ...
  except formula provided by [Son1990]_ (and [Son1994]_,but with fewer digits):
...
def tdew(e, p=None, hPa=False, Kelvin=True):
  ...
  .. math::
    ...
    t = (243.12 * np.log(e / (6.112 * _f(p)))) / (17.62 - np.log(e / (6.112 * _f(p))))
  ...
  if np.isnan(e):
    ...
    t = (243.12 * np.log(e / (6.112 * _f(p)))) / (17.62 - np.log(e / (6.112*_f(p)))) # C
...
def tfrost(e, p=None, hPa=False, Kelvin=True):
  ...
  .. math::
    ...
    t = (272.62 * np.log(e / (6.112 * f(p)))) / (22.46 - np.log(e / (6.112 * f(p))))
  ...
  if np.isnan(e):
    ...
    t = (272.62 * np.log(e / (6.112 * _f(p)))) / (22.46 - np.log(e / (6.112 * _f(p)))) # C
...
def psychro(t, tw, Kelvin=None, p=None, hPa=False):
  ...
  e = esat_w(tw, Kelvin=False, p=p, hPa=True) - 6.53E-4 * (1 + 0.000944 * tw) * p1 * (t - tw) # hPa
...
def psychro_ice(t, ti, Kelvin=None, p=None, hPa=False):
  ...
  e = esat_i(ti, Kelvin=False, p=p, hPa=True) - 5.75E-4 * p1 * (t - ti) # hPa
...
class Humidity(object):
  ...
  def __init__(self, t=None, p=None,
    ...
    num = { x:loc[x] for x in ['e','rh','td','tf','t','tw','ti','q','m','p','rhow'] }
    if _only(num,['rh','t'],['p']):
      if percent == True:
      ...
      raise ValueError('invalid combination of parameters: {}'.format(' '.join(k for k,v in loc.items() if v is not None)))
  ...
  def td(self, Kelvin=None):
    ...
    if Kelvin is None or Kelvin == True:
  ...
  def tw(self, Kelvin=None):
    ...
    .. math::
      tw' &= tl - \\frac{ ew(tw_i) - e}{ 6.53E-4 * (1 + 0.000944 * tw_i) * p} \\\\
    ...
    for i in range(100):
      twl = tw
      twn = tl - (esat_w(tw,Kelvin=False,hPa=True) - ee) / ( 6.53E-4 * (1 + 0.000944 * tw) * pp) # °C
      tw = tw + 0.5 * (twn - tw)
      if abs(tw -twl) <= 0.01 :
        break
    else:
  ...
  def ti(self, Kelvin=None):
    ...
    :param Kelvin: (optional) if ``False``, all temperatures are assumed to
      ...
      If missing of ``None``, unit  temperatures are autodetected. Defaults to
      ``None``.
    :return: ice / frozen wet-bulb temperatur (in K or °C, depending on `Kelvin`).
    :rtype: float
    :raises ValueError: if `Kelvin` is not ``True``, ``False``, None,or missing
    ...
    for i in range(100):
      til = ti
      tin = tl - (esat_i(ti,Kelvin=False,hPa=True) - ee) / ( 5.75E-4 * pp) # °C
      ti = ti + 0.5 * (tin - ti)
      if abs(ti -til) <= 0.01 :
        break
    else:
  ...
  def tequipot(self, Kelvin=None):
    ...
    equipotential temperature is the temperature of an air parcel from which
    all the water vapor has been removed by an adiabatic process [Stu1988]_:

    .. math:: \\Theta_{e} = T_e \\left( \\frac{p_0}{p} \\right) ^{\\frac{R}{c_p}}
  ...
  def rhow(self):
    ...
    '''
    rw = gas_rho(self.e,self.t,gas='water',Kelvin=self.Kelvin,hPa=self.hPa)
