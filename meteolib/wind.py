...
"""
import numpy as np
from .constants import kappa, gn
...
class LogWind(object):
  ...
  '''
  def __init__(self,ust=None, z0=None, d=0., u=None, z=None, u2=None, z2=None):
    loc = locals()
    ...
    if _only(par,['ust','z0'],['d']):
      ...
      if d is None:
        ...
        for i in range(100):
          ...
          zz = np.exp( (u*np.log(z2-displacement_factor*z0l) - u2*np.log(z-displacement_factor*z0l)) / (u - u2) )
          z0 = z0l * (zz/z0l)**0.25
        ...
        u2  = _check('u2',  u2, 'float', gt= u)
        z2  = _check('z2',  z2, 'float', gt= z)
        self.z0 = np.exp( (u*np.log(z2-self.d) - u2*np.log(z-self.d)) / (u - u2) )
        self.ust = u / np.log((z-self.d) / self.z0 )
...
class DiabaticWind(object):
  ...
  '''
  def __init__(self,ust=None, z0=None, d=0., u=None, z=None, LOb=None, zoL=None):
    loc = locals()
...
#        self.d = d
  ...
  def u(self,z):
    ...
    '''
    z = _check('z', z, 'float', ge=0.)
    u = (self.ust / kappa) * (np.log((z-self.d)/self.z0) - psi_m(self.zoL))
    return u
...
def vectormean(ff=None,dd=None,pairs=None):
  ...
  :param ff: wind speed in any unit, requires `dd` (array-like, same lenght as `dd`)
  :param dd: wind direction in degrees, requires `ff` (array-like, same lenght as `ff`)
  :param pairs: pairs of wind speed (any unit) and direction (in degrees) (list of tuples)
