...
from .constants import Ttriple,R,Rw,Rco2
...
def gas_p(rho,T,q=None,Kelvin=None,hPa=False,gkg=False,gas='air'):
  ...
  :param T:  gas temperature in :math:`K` or :math:`^\circ C` , depending on `Kelvin` .
  :param rho: density of gas (in :math:`kg/m^3` .
  :param gas: kind of gas.
    One of `dry` (dry air), `moist` (humid air), `water` (water vapor), or `co2` (carbon dioxide).
    Defaults to `dry`.
...
def gas_t(p,rho,q=None,Kelvin=None,hPa=False,gas='air'):
  ...
  :return: gas temperature in :math:`K` or :math:`^\circ C` , depending on `Kelvin` .
  :rtype: float
...
def cpt(t, Kelvin=None):
  ...
  :param t:  air temperature in :math:`K` or :math:`^\circ C` , depending on `Kelvin` .
  :param Kelvin: (optional) if ``False``, all temperatures are assumed to
    be Kelvin. If ``False``, all temperatures are assumed to be Celsius.
    If missing of ``None``, unit  temperatures are autodetected. Defaults to
    ``None``.

  :return: specific heat capacity of **dry air** at constant pressure (in :math:`J kg^-1 K^-1`).
  :rtype: float
...
def cpvt(t, Kelvin=None):
  ...
  :return: specific heat capacity of **water vapor** at constant pressure (in :math:`J kg^-1 K^-1`).
  :rtype: float

  .. math::

    c_{pv} ~=~ 1858 + 3.820 \\times 10^{-1} T + 4.220 \\times 10^{-4} T^2 - 1.996 \\times 10^{-7} T^3

  "should be accurate for all near-surface atmospheric temperatures" [And2006]_.
...
def cwt(t, Kelvin=None):
  ...
  :return: specific heat capacity of **liquid water** at constant pressure (in :math:`J kg^-1 K^-1`).
  :rtype: float

  .. math::

    c_{w} ~=~ 4217.4 - 3.720283 T^2 - 2.654387 \\times 10^{-3} T^3 + 2.093236 \\times 10^{-5} T^4
...
def cit(t, Kelvin=None):
  ...
  :return: specific heat capacity of **ice** at constant pressure (in :math:`J kg^-1 K^-1`).
  :rtype: float

  .. math::

    c_{w} ~=~ -114.19 + 8.1288 T + 3.421 T~ \\exp \\left[ -(T/125.1)^2 \\right]
...
def Lvt(t, Kelvin=None, source="andreas"):
  ...
  andreas
    ...
    :math:`L_{v} ~=~ \\left( 25.00 - 0.02274 \\vartheta \\right) \\times 10^{5}`
