...
def _check_scalar(name,par,kind,lt=None,le=None,ne=None,ge=None,gt=None,nan=True,none=False):
    ...
    if kind in ['int','float','datetime']:
        ...
        if nan == False and    np.isfinite(val) == False:
...
def _check(name,par,kind,lt=None,le=None,ne=None,ge=None,gt=None,nan=True,none=False):
    if par is None or pd.api.types.is_scalar(par):
            ...
            for i,p in enumerate(par):
                    res[i] = _check_scalar(name,p,kind,lt,le,ne,ge,gt,nan,none)
            return res
