...
from .constants import sigma,Tzero
...
def penmanmonteith_mono(T, rh, vh, p, Kdn, time=None, df=None, 
  ...
  :param T:  2-m temperature (series) in :math:`K` or :math:`^\circ C`, depending on `Kelvin` .
  :param rh: realtive humidity (series) in percent or fractions of unity, depending on `Kelvin` .
  ...
  :param lat: Latitude (in degrees) of the location for which evaportaion is calculated.
    Defaults to 45°(N).
  :param ele: Elevation (above sea level) of the location for which evaportaion is calculated.
    Defaults to 0m.
  ...
  if all(isinstance(x,str) for x in [T,rh,vh,p,Kdn]):
    if isinstance(df, pd.DataFrame):
      ...
      raise ValueError('string arguments require a pandas.DataFrame as df')
    ...
    if time is None:
      raise ValueError('list arguments require list of observation times as time')
    o = pd.DataFrame({'T':T,
                     'rh':rh,
                     'vh':vh,
                     'p':p,
  ...
  cur['slope'] = (4098*
                  (0.6108*np.exp((17.27*cur['Tmean'])/(237.3+cur['Tmean'])))/
                  ((237.3+cur['Tmean'])**2)
                 )
  ...
  cur['delta'] = cur['slope']/(cur['slope']+cur['gamma']*(1+0.34*cur['umean']))
  ...
  cur['e_Tmax'] = 0.6108*np.exp((17.27*cur['Tmax'])/(237.3+cur['Tmax']))
  cur['e_Tmin'] = 0.6108*np.exp((17.27*cur['Tmin'])/(237.3+cur['Tmin']))
  cur['es'] = (cur['e_Tmax']+cur['e_Tmin'])/2.
  # 11. Actual vapor pressure (ea) derived from relativehumidity
  cur['ea'] = (cur['e_Tmax']*cur['rhmax']/100.0 + cur['e_Tmin']*cur['rhmin']/100.0) / 2.
...
def penmanmonteith(T, rh, vh, p, Kdn, time=None, df=None,
  ...
  Code using otherlibrary functions, based on the steps described by [Zot2018]_
  to implement the method as described in [FAO56]_ .
  ...
  cur['umean'] = o['vh'].resample('1D').mean()
  # 4. Slope of saturation vapor pressure curve 
  cur['slope'] = [((esat_w(x+0.1)-esat_w(x-0.1))/0.2) / 1000. for x in cur['Tmean']] # Pa/K -> kPa/K
  # 5. Atmospheric Pressure (P)
  cur['pmean'] = o['p'].resample('1D').mean()/10.
  # 6. Psychrometric constant 
  cur['gamma'] = 0.000665*cur['pmean']
