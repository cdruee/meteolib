...
class Test_dir2uv(unittest.TestCase):
  ...
  def test_speeds(self):
    with self.assertRaises(ValueError): m.wind.dir2uv(-1., 0.)
    ...
    for s,f in zip(res,ref):
      self.assertAlmostEqual(s,f, places=4)
  def test_shape(self):
    with self.assertRaises(ValueError): m.wind.dir2uv([0], [0,0])
    res,ref = m.wind.dir2uv([2.], [45.]),( -1.4142,  -1.4142)
    for s,f in zip(res,ref):
      self.assertAlmostEqual(s,f, places=4)
    res,ref = m.wind.dir2uv([2.,2.], [45.,45.]),( [-1.4142,  -1.4142], [-1.4142,  -1.4142] )
    for s,f in zip(res,ref):
      for a,b in zip (s,f):
